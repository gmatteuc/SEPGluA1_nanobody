...
% NaN-aware L+R and |L-R| along the LR axis
% LR axis is set as the dimension of length =800.
% Midline (if odd width/height along LR) is set to 0 
%
% run as:
%   lr_sum_and_diff_auto('input.tif','sum_lr.tif','absdiff_lr.tif')
    ...
    % --- Determine LR axis: the dimension equal to 800 ---
    ...
    % Helper: set tags from a sample frame
    function set_tags(t, frame)
        r = size(frame,1); c = size(frame,2);
        if ndims(frame) >= 3, sp = size(frame,3); else, sp = 1; end
        tag.ImageLength          = r;
        tag.ImageWidth           = c;
    ...
    try
        for z = 1:N
            ...
            F = flip(I, xDim);               % mirror along LR axis
